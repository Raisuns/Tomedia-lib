...
<div class="row">
      ...
      <form action="" method="POST" class="book_borrower_form">
        ...
        <table class="table">
          ...
          <tr>
            ...
            @elseif($book_user->status === 4)
            <td class="text-info text-wrap">Dikembalikan</td>
            @endif
          </tr>
        </table>
    </div>
  ...
  <div class="col-lg-5">
    ...
    <div class="py-4">
      ...
      @elseif($book_user->status === 1)
      <button type="submit" class="btn btn-warning" data-id="{{ $book_user->id }}" id="book-return-button">Dikembalikan</button>
      @endif
    </div>
...
<script>
  $(document).ready(function() {
    $("#book-swal-show-button").click(function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#book_image_detail_show").attr("src", url + image);
    ...
    $("#book-return-button").click(function(e) {
      e.preventDefault();
      let button = $(this);
      let id = button.data("id");
      let token = $("input[name=_token]").val();
      ...
      Swal.fire({
        ...
        if (result.value) {
          $.ajax({
            url: "{{ route('admin.json-book.return', $book_user->id) }}",
            type: "PUT",
            data: {
              id: id,
              _token: token
    ...
    $("#user-swal-show-button").click(function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#user_image_detail_show").attr("src", url + image);
