...
class JsonResponseController extends Controller
{
    ...
    public function returnBookBorrower($id)
    {
        $book_user = BookUser::find($id);

        if ($book_user->status == 1) {
            $book = Book::find($book_user->book_id);
            $book->where('id', $book_user->book_id)->update(['stock' => $book->stock + 1]);
        }

        $book_user->where('id', $id)->update(['status' => 4]);

        return response()->json(['message' => 'Berhasil mengembalikan buku']);
    }
}
